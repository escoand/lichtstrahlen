...
/**
 * Implements a general way of dealing with notes for dates. Save the notes as a
 * serialized HashMap.
 * 
 * @author escoand
 */
public class Notes extends Object {
	...
	private void _load() {
		try {
			ObjectInputStream stream = new ObjectInputStream(
					new BufferedInputStream(context.openFileInput("notes.ser")));
		...
		try {
			ObjectOutputStream stream = new ObjectOutputStream(
					new BufferedOutputStream(context.openFileOutput(
							"notes.ser", Context.MODE_PRIVATE)));
	 ...
	 * @param date
	 *            the date
	 * @return the note for the specified date, or null if no note for the
	 *         specified date is found.
	 */
