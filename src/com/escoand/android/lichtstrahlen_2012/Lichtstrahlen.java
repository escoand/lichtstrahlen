...
public class Lichtstrahlen extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// init
		scale = getSharedPreferences(getString(R.string.app_name), MODE_PRIVATE)
				.getInt("scale", 100);
		((WebView) findViewById(R.id.content)).getSettings().setSupportZoom(
				true);
		((WebView) findViewById(R.id.content)).getSettings()
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_ABOUT_ID:
			...
			try {
				...
				((TextView) dialog.findViewById(R.id.txtAbout)).setText(Html
						.fromHtml(getString(R.string.about)),
	...
	public class VerseTask extends
			AsyncTask<Void, Void, HashMap<String, String>> {
		private final SimpleDateFormat yearmonth = new SimpleDateFormat(
				"yyyyMM");
		private final String datestring = new SimpleDateFormat("yyyyMMdd")
				.format(date);
		...
		protected void onPostExecute(HashMap<String, String> result) {
			...
			if (result.containsKey("monthtext")) {
				...
				content += "<div style=\"font-weight: bold;\">"
						+ TextUtils.htmlEncode(getString(R.string.mainMonth))
						+ "</div>";
				content += "<div>"
						+ TextUtils.htmlEncode(result.get("monthtext"))
						+ "</div>";
				content += "<div style=\"text-align: right; white-space: nowrap;\">"
						+ TextUtils.htmlEncode(result.get("monthverse"))
						+ "</div>";
				content += "</p>";
			...
			if (result.containsKey("weektext")) {
				content += "<p>";
				content += "<div style=\"font-weight: bold;\">"
						+ TextUtils.htmlEncode(getString(R.string.mainWeek))
						+ "</div>";
				content += "<div>"
						+ TextUtils.htmlEncode(result.get("weektext"))
						+ "</div>";
				content += "<div style=\"text-align: right; white-space: nowrap;\">"
						+ TextUtils.htmlEncode(result.get("weekverse"))
						+ "</div>";
				content += "</p>";
			...
			if (result.containsKey("text")) {
				...
				content += "<div style=\"float: right; text-align: right; white-space: nowrap;\">"
						+ TextUtils.htmlEncode(result.get("verse")) + "</div>";
				content += "<div style=\"font-weight: bold;\">"
						+ TextUtils.htmlEncode(result.get("header")) + "</div>";
				content += "<div style=\"clear: both;\">"
						+ TextUtils.htmlEncode(result.get("text")) + "</div>";
				content += "<div style=\"text-align: right;\">"
						+ TextUtils.htmlEncode(result.get("author")) + "</div>";
				content += "</p>";
				...
				if (notes.exist(date)) {
					...
					content += "<div style=\"font-weight: bold;\">"
							+ TextUtils
									.htmlEncode(getString(R.string.mainNote))
							+ "</div>";
					content += "<div>" + TextUtils.htmlEncode(notes.get(date))
							+ "</div>";
					content += "</p>";
				}
			} else {
			...
			if (((WebView) findViewById(R.id.content)).getUrl() != null)
				scale = (int) (((WebView) findViewById(R.id.content))
						.getScale() * 100);
			((WebView) findViewById(R.id.content)).setInitialScale(scale);
			...
			content = "<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"></head><body>"
					+ content + "</body></html>";
			((WebView) findViewById(R.id.content)).loadData(content,
					"text/html", "utf-8");
	...
	private class NotesListTask extends
		...
		protected ArrayList<HashMap<String, String>> doInBackground(
			...
			final SimpleDateFormat yearmonthday = new SimpleDateFormat(
					"yyyyMMdd");
							...
							while (xml.getEventType() != XmlPullParser.END_DOCUMENT) {
								...
								case XmlPullParser.START_TAG:
									...
									// read verse if all or note for day exists
											...
											&& notes.exist(yearmonthday
													.parse(dateread))) {
										xml.next();
										HashMap<String, String> item = new HashMap<String, String>();
										item.put("date", dateformat
												.format(yearmonthday
														.parse(dateread)));
										item.put("verse", xml.getText());
										item.put("note", notes.get(yearmonthday
												.parse(dateread)));
										list.add(item);
									}
									break;
		...
		protected void onPostExecute(ArrayList<HashMap<String, String>> result) {
			...
			selection = new AlertDialog.Builder(Lichtstrahlen.this)
					.setCancelable(true)
					.setTitle(getString(R.string.menuNotes)).setAdapter(
							new SimpleAdapter(Lichtstrahlen.this, result,
									R.layout.noteslist, new String[] { "verse",
											"date", "note" }, new int[] {
								...
								public void onClick(DialogInterface dialog,
									...
									try {
										...
										// go to date
											...
											HashMap<String, String> element = (HashMap<String, String>) selection
													.getListView()
	...
	private class VerseListTask extends
							...
							case XmlPullParser.START_TAG:
								...
								if (xml.getName().equals("entry")) {
									...
									if (xml.getAttributeValue(null, "until") != null)
										dateuntil = format_yearmonthday
												.parse(xml.getAttributeValue(
														null, "until"));
									else
										dateuntil = dateread;
								}
		...
		protected void onPostExecute(ArrayList<HashMap<String, String>> result) {
			// create list dialog
			selection = new AlertDialog.Builder(Lichtstrahlen.this)
					.setCancelable(true).setTitle(getString(R.string.menuList))
					.setAdapter(
							new VerseListAdapter(Lichtstrahlen.this, result,
									R.layout.list, new String[] { "verse",
											"date" }, new int[] {
											R.id.listVerse, R.id.listDate }),
							new DialogInterface.OnClickListener() {
								@Override
